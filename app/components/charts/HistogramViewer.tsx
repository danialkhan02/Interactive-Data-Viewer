      ...
      <Space direction="vertical" className="w-full" size="large">
        ...
        <div className="text-center">
          <AntTitle level={3} className="mb-2">
            {filteredCount > 0 ? 'Successful Experiment Distribution' : 'No Experiments Found'}
          </AntTitle>
          {filteredCount > 0 ? (
            <>
            ...
            <>
              <Text className="text-lg" type="secondary">
                No experiments have {selectedOutput} values in your target range
              </Text>
              <br />
              <Text type="secondary">
            ...
            <Col xs={24} sm={16}>
              <div>
                   ...
                   <Text type="secondary" className="text-sm">
                     [{outputRange?.[0]?.toFixed(2) || '0'}, {outputRange?.[1]?.toFixed(2) || '0'}] • {filteredCount} experiments
                   </Text>
                 </div>
                 <RangeSlider
                   property={selectedOutput}
                   onChange={handleRangeChange}
                   value={outputRange || undefined}
                 />
              </div>
            </Col>
          </Row>
        ...
        {filteredCount === 0 ? (
          ...
          <div className="text-center py-8">
            <Alert
               message="No experiments match your criteria"
               description={`No experiments have ${selectedOutput} values between ${outputRange?.[0]?.toFixed(2) || '0'} and ${outputRange?.[1]?.toFixed(2) || '0'}. Try adjusting your target range above.`}
               type="warning"
               showIcon
          ...
          // Show charts when experiments are found
          <>
            <Divider />
